        ...
        <div id="header" :class="`${(height > 200) ? 'sticky' : ''} ${($route.fullPath == '/') ? 'front' : ($route.fullPath == '/instructors' ? 'instructor' : 'not_front')} ${($store.state.loginSignUpStatus) ? 'login' : ''}`">
            <div class="navbar">
                ...
                <ul class="nav_list" v-if="!$store.state.isMobile" itemscope itemtype="https://schema.org/SiteNavigationElement">
                    ...
                    <li itemprop="name">
                        <a href="https://shop.riderevolution.ph/" class="nav_item" itemprop="url" target="_blank">Shop</a>
                    </li>
                    <li itemprop="name">
                        <a href="https://ondemand.riderevolution.ph/" class="nav_item" itemprop="url" target="_blank">On-Demand</a>
                    </li>
        ...
        methods: {
                    ...
                    if (me.$store.state.articleAlertStatus) {
                        ...
                        if (me.$store.state.articleAlertStatus) {
                            ...
                            if (document.getElementById('complete_profile')) {
                                ...
                                document.querySelector('.login_sign_up').style.height = `calc(100vh - ${document.getElementById('header').offsetHeight}px)`
                            } else {
            ...
            windowScroll () {
                ...
                setTimeout( () => {
                    if (me.advisory != null) {
                        if (me.$store.state.articleAlertStatus) {
                            document.getElementById('header').style.top = `${document.getElementById('article_alert').offsetHeight}px`
                        } else {
                            document.getElementById('header').style.top = `${0}px`
                        }
                    }
