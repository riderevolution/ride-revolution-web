    ...
    <div class="default_modal">
        ...
        >
            <div class="modal_wrapper">
                <h2 class="form_title alt">
                    {{ category == 'transfer' ? 'Transfer' : 'Share' }} Class
                    Package
                </h2>
                <h3 class="form_subtitle">
                    You are about to {{ category }} your
                    <b>{{ data.class_package.name }}</b>
                    . Please input the recipient's username or email address.
                </h3>
                        ...
                        me.$axios
                            ...
                            .then(res => {
                                if (res.data) {
                                    ...
                                    me.$axios
                                        .post(
                                            `api/packages/class-packages/${me.category}?forWeb=1`,
                                            formData,
                                            {
                                                headers: {
                                                    Authorization: `Bearer ${token}`
                                        ...
                                        .then(res => {
                                            if (res.data) {
                                                setTimeout(() => {
                                                    ...
                                                    me.$parent.promptMessage = `You have successfully ${
                                                        me.category == 'share'
                                                            ? 'shared'
                                                            : 'transferred'
                                                    } your package.`
                                                    me.$store.state.bookerPromptStatus = true
                                                    me.$parent.$parent.toggleTab(
                                                        2,
                                                        'packages'
                                                    )
                                                }, 500)
