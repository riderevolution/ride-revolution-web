    ...
    <div class="default_modal overlay alt">
        ...
        <div class="confirmation_wrapper">
                ...
                <div
                    :class="
                        `form_custom_checkbox ${
                            data.shares_count > 1 ? 'share-scroll' : ''
                        }`
                    "
                >
                    <div
                        ...
                        :key="key"
                        :id="`user_${key}`"
                        :class="['custom_checkbox', share.active && 'active']"
            ...
            <template v-else-if="active_step == 2">
                ...
                <div class="button_group alt">
                    <div class="flex default_btn_red" @click="active_step = 1">
                        Back
                    </div>
                    <div
                        class="flex default_btn_wht ml"
        ...
        methods: {
            selectUser(payload) {
                this.data.shares.forEach(share => {
                    share.active =
                        share.user_id == payload.user_id ? true : false
                })
                    ...
                    me.$axios
                        ...
                        .then(res => {
                            if (res.data) {
                                ...
                                formData.append(
                                    'receiver_id',
                                    me.selected_user.user_id
                                )
                                me.$axios
                                    .post(
                                        `api/packages/class-packages/unshare`,
                                        formData,
                                        {
                                            headers: {
                                                Authorization: `Bearer ${token}`
