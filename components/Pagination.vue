    ...
    export default {
        ...
        methods: {
            ...
            nextPage (current, last) {
                ...
                if (current != last) {
                    me.loader(true)
                    me.$axios.get(`${me.apiRoute}?page=${current + 1}${(me.$parent.category == 'transactions') ? '&forWeb=1' : ''}`).then(res => {
                        if (me.$parent.category == 'transactions') {
                            me.$parent.transactions = res.data.customer.payments
            ...
            currentPage (count, first) {
                ...
                me.$axios.get(`${me.apiRoute}?page=${count}${(me.$parent.category == 'transactions') ? '&forWeb=1' : ''}`).then(res => {
                    if (me.$parent.category == 'transactions') {
                        me.$parent.transactions = res.data.customer.payments
                    }
                }).catch(err => {
